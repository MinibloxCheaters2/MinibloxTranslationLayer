...
function uuid() {
	return "10000000-1000-4000-8000-100000000000".replace(/[018]/g, (j) =>
		(
			j ^
...
async function connect(client, requeue, gamemode, code) {
	...
	ClientSocket.setUrl(
		`https://${fetched.serverId}.servers.coolmathblox.ca`,
		void 0,
	);
	let session = "";
	try {
	...
	ClientSocket.on(
		...
		/** @param {string} reason **/ (reason) => {
			...
			if (skipKick > Date.now()) {
				console.debug(
					`Server kicked us for ${reason}, ignoring it since skipKick is active.`,
				);
				return;
			}
